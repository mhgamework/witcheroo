...
namespace Modules.MHGameWork.Reusable.UnityAdditions.CustomPropertyDrawers.Editor
{
	[CustomPropertyDrawer(typeof(MinMaxPropertyAttribute))]
	class MinMaxPropertyDrawer : PropertyDrawer
	{
		const string kVectorMinName = "x";
		const string kVectorMaxName = "y";
		const float kFloatFieldWidth = 50f;
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			if (property.propertyType == SerializedPropertyType.Vector2)
			{
				...
				MinMaxPropertyAttribute attr = attribute as MinMaxPropertyAttribute;
